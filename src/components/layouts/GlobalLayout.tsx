...
export function GlobalLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      <div className="w-screen h-screen relative bg-avatar-black">
        {children}
        <div className="absolute inset-0 bg-[url(/assets/bg-base.png)] bg-cover bg-center mix-blend-plus-lighter opacity-21" />
        <div className="absolute inset-0 cs-radial" />
      </div>
      <Link
        href={`${baseUrl}/agent/${locale}`}
        className="rounded-2xl p-2 flex items-center gap-3 mb-4 fixed bottom-4 left-1/2 -translate-x-1/2 mt-4 mx-auto w-fit"
        style={{
      ...
      <div
        className={clsx(
          "fixed bottom-6.75 left-11.75 flex items-center justify-between gap-10.5 whitespace-nowrap text-white"
        )}
      >
